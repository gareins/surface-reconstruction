#ifndef MINIBALL_DYNAMIC_H
#define MINIBALL_DYNAMIC_H
...
public:
  ...
  double              excess (const PointMB& p) const;
    
  // modification
  void                reset(); // generates empty sphere with m=s=0
  bool                push (const PointMB& p);
  void                pop ();
...
public:
  // types
  typedef std::list<PointMB>::iterator         It;
  typedef std::list<PointMB>::const_iterator   Cit;
    
private:
  // data members
  int              d;            // dimension
  std::list<PointMB> L;            // internal point set
...
public:
  ...
  // copy from double*
  PointMB (int dim, const double* p)
    : d (dim), coord(new double[dim])
  {
    for (int i=0; i<d; ++i)
      coord[i] = p[i];
  ...
  // assignment
  PointMB& operator = (const PointMB& p)
  {
    assert (d == p.dim());
    if (this != &p)
      for (int i=0; i<d; ++i)
...
void Miniball::check_in (const PointMB& p)
{
  assert (d == p.dim());
  L.push_back(p);
}
...
PointMB Miniball::center () const
{
  return PointMB(d, B.center());
}
...
double Miniball_b::excess (const PointMB& p) const
{
...
bool Miniball_b::push (const PointMB& p)
{
  int i, j;
  double eps = 1e-32;
  if (m==0) {
...
std::ostream& operator << (std::ostream& os, const PointMB& p)
{
  os << "(";
  int d = p.dim();
  for (int i=0; i<d-1; ++i)
